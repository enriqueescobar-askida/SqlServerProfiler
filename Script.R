...
TraceXmlXlsToCsv(xlsxTibble0, filePath = "Data/00 Scripts start.csv");
rm(xlsxTibble0);
xlsxTibble1 <- ScreenXmlXlsFiles(fileList1);
TraceXmlXlsToCsv(xlsxTibble1, filePath = "Data/01 Agent Niveau 1 - Evelyne made mistake and logged out.csv");
rm(xlsxTibble1);
xlsxTibble2 <- ScreenXmlXlsFiles(fileList2);
TraceXmlXlsToCsv(xlsxTibble2, filePath = "Data/02 Agent N1 Complete Agent Web Services engage.csv");
rm(xlsxTibble2);
...
TraceXmlXlsToCsv(xlsxTibble5, filePath = "Data/05 Sup SAC.csv");
rm(xlsxTibble5);
