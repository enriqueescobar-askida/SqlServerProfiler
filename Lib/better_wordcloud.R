...
tucson.cloud <- ggplot(obama.df,
                       aes(x = freq.dif, y = Spacing)) +
  geom_text(aes(size = obama.txt, label = row.names(obama.df), colour = freq.dif)) +
  ...
  scale_size(range = c(3,11), name = "Word Frequency") +
  scale_colour_gradient(low = "darkred", high = "darkblue", guide = "none") +
  scale_x_continuous(breaks = c(min(palin.df$freq.dif),
                                0,
                                max(obama.df$freq.dif)),
...
ggsave(plot = tucson.cloud, filename = "../Data/tucson_cloud.png", width = 13,height = 7)
